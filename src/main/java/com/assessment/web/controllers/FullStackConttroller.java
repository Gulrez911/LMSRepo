...
public class FullStackConttroller {
	...
	public ModelAndView showLogin(@RequestParam String workspace, HttpServletRequest request,
	...
	public @ResponseBody String addQuestionsToSectionAjax(@RequestParam String userName,
			@RequestParam String stackName, @RequestParam String questionId, @RequestParam String testId,
		...
		if (stackName.equals("Java")) {
			...
			json = json.replace("${APP_USER}",
					userName + "=" + testId + "-" + questionId + "-" + System.currentTimeMillis());
			// json = json.replace("${APP_USER}", "a01");
			json = json.replace("${APP_DESC}",
	...
	public @ResponseBody String submitFullStackCode(HttpServletRequest request, HttpServletResponse response,
			@RequestParam String qMapperInstanceId) throws Exception {
		ModelAndView model = new ModelAndView("test");
		User user = (User) request.getSession().getAttribute("user");
		Test test = (Test) request.getSession().getAttribute("test");
		QuestionMapperInstance questionMapperInstance = questionMapperInstanceRep
				.findById(Long.parseLong(qMapperInstanceId)).get();
		...
		// String projname =
		String workSpaceFolder = questionMapperInstance.getWorkspaceUrl().substring(
				questionMapperInstance.getWorkspaceUrl().indexOf("che/") + 4,
				questionMapperInstance.getWorkspaceUrl().length());
		String codebasePath = propertyConfig.getFullStackCodeLocation() + File.separator + workSpaceId
				+ File.separator + workSpaceFolder;
		 ...
		 */
		String analysisFile = "";
		if (questionMapperInstance.getQuestionMapper().getQuestion().getFullstack().getStack()
				.equalsIgnoreCase(FullStackOptions.JAVA_FULLSTACK.getStack())) {
			System.out.println("doing code quality on java stack");
			analysisFile = FileUtils
					.readFileToString(new File(propertyConfig.getSonarAnalysisFileLocation()));
		} else if (questionMapperInstance.getQuestionMapper().getQuestion().getFullstack().getStack()
				.equalsIgnoreCase(FullStackOptions.PHP_FULLSTACK.getStack())) {
			System.out.println("doing code quality on php stack");
			analysisFile = FileUtils
					.readFileToString(new File(propertyConfig.getSonalAnalysisFilePHPLocation()));
		} else if (questionMapperInstance.getQuestionMapper().getQuestion().getFullstack().getStack()
				.equalsIgnoreCase(FullStackOptions.ANGULARJS_FULLSTACK.getStack())) {
			System.out.println("doing code quality on php stack");
			analysisFile = FileUtils.readFileToString(
					new File(propertyConfig.getSonarAnalysisFileAngularLocation()));
		} else if (questionMapperInstance.getQuestionMapper().getQuestion().getFullstack().getStack()
				.equalsIgnoreCase(FullStackOptions.JAVASCRIPT_FULLSTACK.getStack())) {
			System.out.println("doing code quality on php stack");
			analysisFile = FileUtils.readFileToString(
					new File(propertyConfig.getSonarAnalysisFileAngularLocation()));
		} else if (questionMapperInstance.getQuestionMapper().getQuestion().getFullstack().getStack()
				.equalsIgnoreCase(FullStackOptions.DOTNET_FULLSTACK.getStack())) {
			System.out.println("doing code quality on dot net stack");
			analysisFile = FileUtils.readFileToString(
					new File(propertyConfig.getSonarAnalysisFileDotNetLocation()));
		} else {
		...
		try {
			...
			// String[] command =
			// {"/opt/softwares/sonar-scanner-3.3.0.1492/bin/sonar-scanner"};
		...
		String html = FileUtils
				.readFileToString(new File(propertyConfig.getCodeQualityEmailTemplateLocation()));
		...
		html = html.replace("{PROJECT_URL}", questionMapperInstance.getUsageDocumentUrl() == null ? ""
				: questionMapperInstance.getUsageDocumentUrl());
		...
		TestCasesMetric casesMetric = initiateGenericAutomation(workSpaceId, request, questionMapperInstance,
				url);
		...
		html = html.replace("${BEHAVIOUR}", "");// just make sure this is not there for tests not having
							// automation
		EmailGenericMessageThread client = new EmailGenericMessageThread("jatin.sutaria@thev2technologies.com",
				"Code quality Report Link for " + user.getFirstName(), html, propertyConfig);
		String cc[] = { reviewer };
	...
	private TestCasesMetric initiateGenericAutomation(String workspaceId, HttpServletRequest request,
			QuestionMapperInstance questionMapperInstance, String codeQualityUrl) throws IOException {
		User user = userService.findByPrimaryKey(questionMapperInstance.getUser(),
				questionMapperInstance.getCompanyId());
		String baseCodePath = propertyConfig.getFullStackCodeLocation();
		String fin = questionMapperInstance.getWorkspaceUrl() != null
				? (questionMapperInstance.getWorkspaceUrl()
						.substring(questionMapperInstance.getWorkspaceUrl().lastIndexOf("/")
								+ 1, questionMapperInstance.getWorkspaceUrl().length()))
				: "";
		...
		if (!output.exists()) {
			...
			casesMetric.setProblemStatement(
...
//		 }
	...
	private TestCasesMetric initiateAutomation(String workspaceId, HttpServletRequest request,
			QuestionMapperInstance questionMapperInstance, String codeQualityUrl) throws IOException {
		User user = userService.findByPrimaryKey(questionMapperInstance.getUser(),
				questionMapperInstance.getCompanyId());
		if (workspaceId.contains("psk2y2afb3ecogbh")) {
			...
			String path = "/root/.che-multiuser/instance/data/workspaces/workspacepsk2y2afb3ecogbh/regex_jdbc";
			String outputFile = "" + System.currentTimeMillis() + ".txt";
			String[] command = ("/usr/bin/mvn -Dtest=com.assignment.regex.TestPatternFinder test --log-file "
					+ outputFile).split("\\s+");
			System.out.println(command);
			ProcessBuilder builder = new ProcessBuilder();
			builder.command(command);
			...
			String line1 = "noOfTestCases$$$" + noOfTestCases;
			String line2 = "noOfTestCasesFailed$$$" + noOfTestCasesFailed;
			String pb = questionMapperInstance.getQuestionMapper().getQuestion().getQuestionText()
					.replaceAll("\n", "<br/>").replace("\r", "");
			String line3 = "problem$$$" + pb;
			String line4 = "name$$$" + user.getFirstName() + " " + user.getLastName();
			String line5 = "testName$$$" + questionMapperInstance.getTestName();
			...
			String path = "/root/.che-multiuser/instance/data/workspaces/workspacezigca3iu5ynpydhp/JohnDoe-48-47-1568040746461";
			// String path =
			// "/root/.che-multiuser/instance/data/workspaces/workspacek9jw1ghhtr724g7q/console-java-simple";
			String outputFile = "" + System.currentTimeMillis() + ".txt";
			String[] command = ("/usr/bin/mvn -Dtest=com.problem1.TestProblem1 test --log-file "
					+ outputFile).split("\\s+");
			System.out.println("command is " + command.toString());
			ProcessBuilder builder = new ProcessBuilder();
			builder.command(command);
	...
	public @ResponseBody String doUpload(@RequestParam("addimage") MultipartFile addimage,
		...
		if (addimage != null) {
			String fileName = qMapperInstanceId
					+ (user.getFirstName() + user.getLastName() + System.currentTimeMillis())
					+ addimage.getOriginalFilename();
			String destination = propertyConfig.getFileServerLocation() + File.separator + "docs"
	...
	public ModelAndView multifileresults(@RequestParam String workspace, HttpServletRequest request,
		...
		mav.addObject("testGiver",
				lines.get(3).substring(lines.get(3).indexOf("$$$") + 3, (lines.get(3).length())));
		mav.addObject("problemStatement",
				lines.get(2).substring(lines.get(2).indexOf("$$$") + 3, (lines.get(2).length()))
						.replaceAll("\n", "<br/>"));
		mav.addObject("testName",
				lines.get(4).substring(lines.get(4).indexOf("$$$") + 3, (lines.get(4).length())));
		mav.addObject("codeQuality",
				lines.get(5).substring(lines.get(5).indexOf("$$$") + 3, (lines.get(5).length())));
		mav.addObject("usageLink",
				lines.get(6).substring(lines.get(6).indexOf("$$$") + 3, (lines.get(6).length())));
		mav.addObject("noOfTestCases",
				lines.get(0).substring(lines.get(0).indexOf("$$$") + 3, (lines.get(0).length())));
		mav.addObject("noOfTestCasesFailed",
				lines.get(1).substring(lines.get(1).indexOf("$$$") + 3, (lines.get(1).length())));
		return mav;
	}
