...
public class ReportsController {
	...
	private List<UserTrait> generateTraits(String companyId, List<QuestionMapperInstance> answers) {
		List<CandidateProfileParams> candidateProfileParams = candidateProfileParamsService
				.findCandidateProfileParamsByCompanyId(companyId);
	...
	private List<QuestionMapperInstance> getAnswersFromLastUserAttempts(String courseContext, String email,
			String companyId) {
		List<String> tests = questionMapperInstanceService.findUniqueTestsForCourseContext(courseContext, email,
				companyId);
		List<QuestionMapperInstance> instances = new ArrayList<>();
		for (String test : tests) {
			List<String> users = questionMapperInstanceService.findUniqueUsersForCourseContextAndTest(test,
					courseContext, email, companyId);
			if (users != null && users.size() > 0) {
				String usr = users.get(users.size() - 1);
				List<QuestionMapperInstance> ins = questionMapperInstanceService
						.findQuestionMapperInstancesForUserForTest(test, usr, companyId);
	...
	public ModelAndView showComprehensiveReportForCourse(@RequestParam String companyId,
			@RequestParam String courseContext, @RequestParam String email, HttpServletRequest request,
		...
		// questionMapperInstanceService.findQuestionMapperInstancesForUserForCourseContext(courseContext,
		// email, user.getCompanyId());

		/**
		 * Gets the answers of the last user attempt for every test.
		 */
		List<QuestionMapperInstance> instances = getAnswersFromLastUserAttempts(courseContext, email,
				companyId);
		...
		System.out.println("email is " + email + " course context is " + courseContext + " total answers "
				+ instances.size());
		/**
		...
		for (QuestionMapperInstance ans : instances) {
			...
			if (map_weight_totalsQs.get(ans.getQuestionMapper().getQuestion().getCourseWeight()) == null) {
				...
				Integer totalQsForWeight = map_weight_totalsQs
						.get(ans.getQuestionMapper().getQuestion().getCourseWeight());
				totalQsForWeight++;
				map_weight_totalsQs.put(ans.getQuestionMapper().getQuestion().getCourseWeight(),
						totalQsForWeight);
			}

			if (ans.getQuestionMapper().getQuestion().getQuestionType().getType()
					.equals(QuestionType.MCQ.getType())) {
				mcqs.add(ans);
				if (ans.getCorrect()) {
					correctAnswers++;
					if (map.get(ans.getQuestionMapper().getQuestion().getCourseWeight()) == null) {
						...
						Integer score = map.get(ans.getQuestionMapper().getQuestion()
								.getCourseWeight());
						score++;
						map.put(ans.getQuestionMapper().getQuestion().getCourseWeight(), score);
					}
				}
			} else if (ans.getQuestionMapper().getQuestion().getQuestionType().getType()
				...
				if (totalTestCases != null && passed != null) {
					if ((100 * passed / totalTestCases) >= 50) {
						ans.setCorrect(true);
						correctAnswers++;
						if (map.get(ans.getQuestionMapper().getQuestion()
								.getCourseWeight()) == null) {
							map.put(ans.getQuestionMapper().getQuestion().getCourseWeight(),
									1);
						} else {
							Integer score = map.get(ans.getQuestionMapper().getQuestion()
									.getCourseWeight());
		...
		if (fsapps.size() > 0) {
			...
			for (QuestionMapperInstance instance : fsapps) {
				...
				codeMetrics.setProblemDesc(
						instance.getQuestionMapper().getQuestion().getQuestionText());
...
//    }
	...
	public ResponseEntity<InputStreamResource> downloadUserSessionReportsForTest(@RequestParam String testName,
		...
		try {
			...
			List<QuestionMapperInstance> instances = rep.findQuestionMapperInstancesForUserForTest(testName,
					email, companyId);
			...
			for (QuestionMapperInstance instance : instances) {
				...
				if (test.getConsiderConfidence() == null || !test.getConsiderConfidence()) {
					...
					answerData.setConfidentAboutAnswer(instance.getConfidence() == null ? "No"
							: (instance.getConfidence() ? "Yes" : "No"));
				}
				...
				answerData.setQuestionCategory(
						instance.getQuestionMapper().getQuestion().getQuestionCategory());
				...
				answerData.setCorrectChoice(
						instance.getQuestionMapper().getQuestion().getRightChoices());
			...
			for (AssessmentUserPerspectiveData data : collection) {
				if (data.getTestName().equals(testName)) {
					...
					data.setUrlForUserSession(propertyConfig.getBaseUrl()
							...
							+ URLEncoder.encode(testName) + "&companyId="
		...
		try {
			...
			Collections.sort(dtos, new Comparator<UserBySkillDTO>() {
				...
				public int compare(UserBySkillDTO o1, UserBySkillDTO o2) {
					...
					return (int) (Float.parseFloat(o2.getScoreInPercentage())
							- Float.parseFloat(o1.getScoreInPercentage()));
				}
			});
